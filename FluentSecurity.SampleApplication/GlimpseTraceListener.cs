	...
	public class GlimpseTraceListener : TraceListener
	{
		private static Stopwatch FirstWatch
		{
			get
			{
				...
				if (currentContext != null)
				{
					...
					firstWatch = store[GlimpseTrace.FirstWatchStoreKey] as Stopwatch;

					if (firstWatch != null) return firstWatch;

					store[GlimpseTrace.FirstWatchStoreKey] = firstWatch = new Stopwatch();
					firstWatch.Start();
				}
				return firstWatch;
			}
			set
			{
				...
				currentContext.Items[GlimpseTrace.FirstWatchStoreKey] = value;
				value.Start();
			}
		}
		...
		private static Stopwatch LastWatch
		{
			get
			{
				...
				{
					var store = currentContext.Items;
					lastWatch = store[GlimpseTrace.LastWatchStoreKey] as Stopwatch;

					if (lastWatch != null) return lastWatch;

					store[GlimpseTrace.LastWatchStoreKey] = lastWatch = new Stopwatch();
					lastWatch.Start();
				}
			...
			set
			{
				...
				currentContext.Items[GlimpseTrace.LastWatchStoreKey] = value;
				value.Start();
			}
		}
		...
		private static IList<IList<string>> Messages
		{
			get
			{
				...
				{
					var store = currentContext.Items;
					messages = store[GlimpseTrace.TraceMessageStoreKey] as IList<IList<string>>;

					if (messages != null) return messages;

					store[GlimpseTrace.TraceMessageStoreKey] = messages = new List<IList<string>>
                                                                       {
                                                                           new List<string>
                                                                               {
                                                                                   "Category",
